...
export function createPublishedCurriculumService(options = {}) {
  ...
  function metadataForHub(rows) {
    ...
    return list.find((row) => (
      (row?.hub_code || row?.hubCode) === hubCode
      && (row?.course_key || row?.courseKey) === courseKey
    )) || null;
  }
  ...
  async function fallback(reason, packageVersion) {
    ...
    if (typeof loadBundled !== "function") {
      ...
      if (cached?.package && validator.validate(cached.package).valid) {
        ...
        return remember({ source: "cache", package: cached.package, state: setState(state), publication: state.publication });
      }
  ...
  async function load(packageVersion, { revalidate = false } = {}) {
    ...
    if (!packageVersion && cached?.package && validator.validate(cached.package).valid) {
      try {
        ...
        /* Metadata lookup failed; fall through to the full package RPC. */
      }
    }
  ...
  function loadLatest() {
    ...
    inflight = load(undefined).finally(() => { inflight = null; });
    return inflight;
  }
  ...
  return Object.freeze({
    ...
    refresh: () => load(undefined, { revalidate: true }),
    invalidate: () => {
      loaded = null;
